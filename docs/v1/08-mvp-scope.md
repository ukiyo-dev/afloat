...
- 规则连续守约 run 计算
- `TEST` 规则正式签署并延续 run
- Overview 规则指标：全部履约时显示履约数（履约规则数 / 正式规则总数）；存在违约时切换为红色违约数，只显示未履约规则数量
- Overview 线程指标：只统计当前日期范围观察窗口重叠的活跃 Item。全部已规划时显示已规划线程（已规划 Item 数 / 当前范围活跃 Item 总数）；存在待规划线程时切换为红色待规划线程，只显示待规划 Item 数；红色状态为 `expired`、`stale` 或 `imbalanced`

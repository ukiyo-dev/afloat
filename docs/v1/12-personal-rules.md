...
Overview 的规则指标在全部正式规则均履约时显示“履约数”及“履约规则数 / 正式规则总数”；存在未履约规则时切换为红色“违约数”，只显示范围内至少有一条 `rule_break` 的正式规则数量。正式规则指 `commitment` 为 `signed` 的规则；同一规则在范围内无论产生多少条 `rule_break`，都只计为一个违约规则。`test_break` 保留在规则账本及规则自身统计中，不影响全局指标。范围归属按 `brokenDate` 判断，不按记录的 `createdAt` 判断。正式规则总数为零时不显示该卡片。

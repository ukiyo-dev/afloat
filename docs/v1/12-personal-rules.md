...
Overview 的“违约数”指标显示为“日期范围内 `rule_break` 数 / 正式规则总数”。正式规则指 `commitment` 为 `signed` 的规则；`test_break` 保留在规则账本及规则自身统计中，不进入全局违约数。范围归属按 `brokenDate` 判断，不按记录的 `createdAt` 判断。

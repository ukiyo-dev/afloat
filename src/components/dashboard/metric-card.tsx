export function Metric({ label, value, subscriptValue, danger, success }: { label: string; value: string; subscriptValue?: string; danger?: boolean; success?: boolean }) {
  return (
    <article className={`metric-box ${danger ? 'border-danger bg-danger/5 shadow-[4px_4px_0_0_rgb(var(--color-danger))]' : success ? 'border-success bg-success/5 shadow-[4px_4px_0_0_rgb(var(--color-success))]' : 'bg-ink/5 shadow-brutal'}`}>
      ...
      <strong className={`font-mono text-3xl font-black whitespace-nowrap ${danger ? 'text-danger' : success ? 'text-success' : 'text-ink'}`}>{value}</strong>
    </article>
  );
}

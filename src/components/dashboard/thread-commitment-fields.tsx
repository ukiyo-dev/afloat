...
export function ThreadCommitmentFields({
  ...
  const [amount, setAmount] = useState(
    expectedMinutes === null ? "" : String(expectedMinutes)
  );
      ...
      <div className={`grid grid-cols-1 gap-4 ${derivedDaily !== null ? "sm:grid-cols-2" : ""}`}>
        ...
        {derivedDaily !== null ? (
          <div className="flex flex-col justify-end border-b-2 border-ink pb-2 font-mono">
            ...
            <strong>{formatDuration(Math.round(derivedDaily))} / {days} DAY</strong>
          </div>
        ) : null}
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        ...
        <div className="flex flex-col gap-1">
          <div className="flex items-center justify-between gap-2">
            ...
            {deadline ? <button type="button" className="font-mono text-xs font-bold text-danger hover:underline" onClick={() => setDeadline("")}>清空</button> : null}
          </div>
          <input className="input-brutal w-full" name="deadline" type="date" value={deadline} onChange={(event) => setDeadline(event.target.value)} />
        </div>
      </div>
    </>
  );
